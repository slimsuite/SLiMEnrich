...
#V1.0.2 - This is the server version of the SLiMEnrich. It doesn't save randomized files as well as matches file.
##############################
...
@-webkit-keyframes mymove {
    50% {color: black;}
                  ...
                  "))
                         ...
                         )), useShinyjs(),theme = shinytheme("yeti"),
...
server <- shinyServer(function(input, output, session){
  ...
  inputDatadomain <-eventReactive(input$run, {
    ...
    if(is.null(PfamFile)){
      hProtein<-read.csv("data/domain.csv",header=TRUE,sep=",")[,c('pfam','accnum')]
    }
  ...
  inputDataELMPfam <-eventReactive(input$run, {
    ...
    if(is.null(ELMPfamFile)){
      Pfam<-read.csv("data/motif-domain.tsv",header=TRUE,sep="\t")[,c(1:2)]
  ...
  potentialDMIs <-eventReactive(input$run, {
    ...
    ELMFile<-input$ELM
    validate(
      need(input$ELM != "", "No SLiM prediction file was found Please upload file and 'Run' again "), errorClass = "myClass"
    )
    #File upload check
    PfamFile<-input$domain
    if(is.null(PfamFile)){
